...
class GCP_DAC_Deployment:
    def __init__(self, deployment_type):
        ...
        self.create_result_url = \
            f"http://{os.environ['GCP_DAC_URL']}/api/{deployment_type}_async/result/create/"
    ...
    def send_payload(self, json_payload):
        resp_json = None
        try:
            response = requests.post(self.create_url, data=json.dumps(json_payload), \
    ...
    def get_results(self, task_id):
        ...
        resp_json = None
        try:
            response = requests.get(self.create_result_url + task_id, headers=self.headers)
            resp_json = response.json()
            app.logger.debug("Response from Dac")
            app.logger.debug(pformat(resp_json))
