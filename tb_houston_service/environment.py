...
from tb_houston_service.models import Environment, EnvironmentSchema
...
def read_all():
    ...
    environment = db.session.query(Environment).order_by(Environment.key).all()
...
def read_one(key):
    ...
    environment = (db.session.query(Environment).filter(Environment.key == key).one_or_none())
    ...
    if environment is not None:
        ...
        data = environment_schema.dump(environment)
        return data, 200
    return abort(404, f"Environment with key {key} not found")
...
def create(environmentDetails):
    ...
    existing_environment = (
        db.session.query(Environment).filter(Environment.key == key).one_or_none()
    )
    ...
    return abort(406, f"Environment already exists")
...
def update(key, environmentDetails):
    ...
    existing_environment = db.session.query(Environment).filter(
            Environment.key == key
    ).one_or_none()
    ...
    return abort(404, f"Environment {key} not found")
...
def delete(key):
    ...
    existing_environment = db.session.query(Environment).filter(Environment.key == key).one_or_none()
