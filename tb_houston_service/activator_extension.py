...
def expand_activator(act, dbsession):
    ...
    act.businessUnit = (
        ...
        .filter(BusinessUnit.id == act.businessUnitId)
        .one_or_none()
    )
    # expand activator metadata
    act_metadata = (
        dbsession.query(ActivatorMetadata)
        .filter(ActivatorMetadata.activatorId == act.id)
        .one_or_none()
    )
    if act_metadata is not None:
        act.activatorMetadata = activatorByURL.expand_activator_metadata(
            act_metadata, dbsession
        )
...
def update_activator_metadata(activator_id, act_metadata, dbsession):
    actMetaDetails = (
        dbsession.query(ActivatorMetadata)
        .filter(ActivatorMetadata.activatorId == activator_id)
        .one_or_none()
    )
    if actMetaDetails:
...
def update_activator_metadata_platforms(platforms, activator_metadata_id, dbsession):
    for platform in platforms:
        platform = (
            dbsession.query(ActivatorMetadataPlatform)
            .filter(
...
def update_activator_metadata_variables(variables, dbsession):
    for variable in variables:
        variableDetails = (
            ...
            .filter(ActivatorMetadataVariable.id == variable.get("id"))
            .one_or_none()
        )
        if variableDetails:
...
def create_activator_associations(extraFields, activator, dbsession):
    ...
    if "activatorMetadata" in extraFields:
        ...
        update_activator_metadata_platforms(
            platforms, activator_metadata["id"], dbsession
        )
    ...
    activator_environment.create_activator_environment(
        activator.id, act_env_list, dbsession
    )
    dbsession.commit()
