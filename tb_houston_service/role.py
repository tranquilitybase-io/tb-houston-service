...
def read_all():
    ...
    role = db.session.query(Role).order_by(Role.name).all()
...
def read_one(name):
    ...
    if role is not None:
        ...
        abort(404, "Role with name {name} not found".format(name=name))
...
def update(name, roleDetails):
    ...
    existing_role = db.session.query(Role).filter(Role.name == name).one_or_none()
