...
def update(oid, subnetModeDetails):
    ...
    existing_subnetMode = db.session.query(SubnetMode).filter(SubnetMode.id == oid).one_or_none()

    if existing_subnetMode is not None:
      db.session.query(SubnetMode).filter(SubnetMode.id == oid).update(subnetModeDetails)
      db.session.commit()
      schema = SubnetModeSchema()
