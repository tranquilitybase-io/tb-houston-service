...
def read_all():
    ...
    fdr = Folder.query.order_by(Folder.id).all()
...
def read_one(oid):
    ...
    fdr = (Folder.query.filter(Folder.id == oid).one_or_none())
    ...
    if fdr is not None:
        ...
        abort(
            404, "Folder with id {id} not found".format(id=oid)
        )
...
def delete(oid):
    ...
    existing_folder = Folder.query.filter(Folder.id == oid).one_or_none()
...
def read_or_create_by_parent_folder_id_and_folder_name(parentFolderId, folderName):
    ...
    if fdr is not None:
        ...
        folder_schema = FolderSchema()
        data = folder_schema.dump(fdr)
        app.logger.debug(f"folder::read_or_create_by_parent_folder_id_and_folder_name: {data}")
        return data, 200
    else:
        folderDetails = {
