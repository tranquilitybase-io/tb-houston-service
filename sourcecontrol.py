...
def read_one(key):
    ...
    sourceControl = (SourceControl.query.filter(SourceControl.key == key).one_or_none())
    ...
    if sourceControl is not None:
        ...
        abort(
            404, "SourceControl with key {key} not found".format(key=key)
        )
...
def create(sourceControlDetails):
    ...
    existing_sourceControl = (
        SourceControl.query.filter(SourceControl.key == key).one_or_none()
    )
...
def delete(key):
    ...
    existing_sourceControl = SourceControl.query.filter(SourceControl.key == key).one_or_none()
