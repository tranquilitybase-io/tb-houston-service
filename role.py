...
def read_all():
    ...
    role = Role.query.order_by(Role.name).all()
...
def read_one(name):
    """
    Responds to a request for /api/role/{key}.
    with one matching role from roles

    :param application:   key of role to find
    :return:              role matching key.
    """
    ...
    if role is not None:
        ...
        abort(
            404, "Role with name {name} not found".format(name=name)
        )
...
def create(roleDetails):
    ...
    existing_role = (
        Role.query.filter(Role.name == roleDetails['name']).one_or_none()
    )
...
def update(name, roleDetails):
    """
    Updates an existing role in the role list.

    :param name:    name of the role to update in the role list
    ...
    if existing_role is not None:
        ...
        update_role.name = roleDetails['name']
...
def delete(name):
    ...
    existing_role = Role.query.filter(Role.name == name).one_or_none()
    ...
    if existing_role is not None:
        ...
        return make_response(f"Role {name} successfully deleted", 200)
